...
$xtpl->assign("row", $row);
//other
$sql1 = "SELECT md5(news_id) AS news_id, news_title, news_image FROM tg_news WHERE news_id <> '" . $row['news_id'] . "' AND news_active = '1' LIMIT 0,4";
$rs1 = execSQL($sql1);

while ($row1 = mysql_fetch_assoc($rs1)) {
    if ($row1['news_image']) {
        if (file_exists("upload/news/" . $row1['news_image'])) {
            $row1['news_image'] = '<img src="upload/news/' . $row1['news_image'] . '" class="media-object" style="width: 64px; height: 64px;" />';
        } else {
            if ($row1['news_image'])
                $row1['news_image'] = "";
        }
